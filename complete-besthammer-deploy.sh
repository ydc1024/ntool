...
# Create bootstrap/app.php FIRST (critical Laravel file)
...
# Create essential Laravel configuration files
...
return [
    ...
    'currency' => [
        ...
        'api_url' => env('CURRENCY_API_URL', 'https://api.exchangerate-api.com/v4/latest/'),
        'fallback_url' => env('CURRENCY_BACKUP_API_URL', 'https://api.fixer.io/latest'),
    ],
];
EOF
...
# Create additional Laravel config files
...
class BmiCalculatorService
{
    ...
    public function analyzeTrends($records)
    {
        if ($records->count() < 2) {
            return ['message' => 'Need at least 2 records to analyze trends'];
        }
        ...
        $trends['weight_change'] = $latest->weight_kg - $previous->weight_kg;
        $trends['bmi_change'] = $latest->bmi - $previous->bmi;
        $trends['direction'] = $trends['weight_change'] > 0 ? 'increasing' : 'decreasing';

        return $trends;
    }

    private function getBodyFatRanges($age, $gender)
    {
        return [
            'essential' => $gender === 'male' ? [2, 5] : [10, 13],
            'athletes' => $gender === 'male' ? [6, 13] : [14, 20],
            'fitness' => $gender === 'male' ? [14, 17] : [21, 24],
            'average' => $gender === 'male' ? [18, 24] : [25, 31],
            'obese' => $gender === 'male' ? [25, 100] : [32, 100]
        ];
    }

    private function categorizeBodyFat($bodyFat, $ranges)
    {
    ...
    private function getBodyFatRecommendations($category)
    {
        $recommendations = [
            'essential' => ['Consult healthcare provider', 'May be too low for optimal health'],
            'athletes' => ['Excellent level for athletes', 'Maintain current routine'],
            'fitness' => ['Good fitness level', 'Continue healthy lifestyle'],
            'average' => ['Normal range', 'Consider increasing activity'],
            'obese' => ['Consult healthcare provider', 'Focus on fat loss strategies']
        ];

        return $recommendations[$category] ?? ['Consult healthcare provider'];
    }

    private function getExerciseSchedule($goal, $activityLevel)
    {
        return [
            'cardio_days' => 3,
            'strength_days' => 2,
    ...
    private function getCardioRecommendations($goal, $age)
    {
        $maxHR = 220 - $age;
        return [
            'target_heart_rate' => round($maxHR * 0.7),
            'max_heart_rate' => $maxHR,
            'recommended_activities' => ['walking', 'cycling', 'swimming']
        ];
    }

    private function getStrengthTrainingPlan($goal)
    {
        return [
            'frequency' => '2-3 times per week',
            'focus_areas' => ['compound movements', 'progressive overload'],
            'exercises' => ['squats', 'deadlifts', 'push-ups', 'rows']
        ];
    }

    private function getFlexibilityRoutine($age)
    {
        return [
            'frequency' => 'daily',
    ...
    private function getProgressionPlan($goal, $activityLevel)
    {
        return [
            'weekly_increase' => '5-10%',
            'plateau_strategy' => 'vary intensity and exercises',
            'rest_importance' => 'essential for recovery'
        ];
    }
}
EOF
...
cat > database/migrations/2024_01_01_000001_create_bmi_records_table.php << 'EOF'
<?php
...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('bmi_records', function (Blueprint $table) {
            ...
            $table->enum('goal', ['lose_weight', 'maintain', 'gain_weight', 'lose_weight_fast', 'gain_weight_fast'])->nullable();
            ...
            $table->integer('bmr');
            $table->integer('tdee');
            $table->decimal('ideal_weight_min', 5, 1);
            $table->decimal('ideal_weight_max', 5, 1);
...
cat > database/migrations/2024_01_01_000002_create_currency_conversions_table.php << 'EOF'
<?php
...
cat > database/migrations/2024_01_01_000003_create_loan_calculations_table.php << 'EOF'
<?php
...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('loan_calculations', function (Blueprint $table) {
            ...
            $table->enum('loan_type', ['fixed', 'variable', 'interest_only'])->default('fixed');
...
info "✓ Complete Models and Migrations created"
...
if command -v composer &>/dev/null && [ -f "composer.json" ]; then
    ...
    if su - $WEB_USER -c "cd '$WEB_ROOT' && composer install --no-dev --optimize-autoloader --no-interaction" 2>/dev/null; then
        info "✓ Composer dependencies installed"
    else
        warning "Composer install failed - trying alternative method"
        # Try with different approach
        cd "$WEB_ROOT"
        composer install --no-dev --optimize-autoloader --no-interaction 2>/dev/null || warning "Composer install completely failed"
        chown -R $WEB_USER:$WEB_USER "$WEB_ROOT/vendor" 2>/dev/null || true
    fi
else
...
API_BMI_TEST=$(curl -s -X POST "http://localhost/api/calculate-bmi" \
    -H "Content-Type: application/x-www-form-urlencoded" \
    -d "height_cm=175&weight_kg=70&age=30&gender=male&activity_level=moderately_active" \
    -w "%{http_code}" -o /dev/null 2>/dev/null || echo "000")

if [[ "$API_BMI_TEST" =~ ^[23] ]]; then
    log "✓ BMI API working (HTTP $API_BMI_TEST)"
else
    warning "BMI API status: HTTP $API_BMI_TEST"
fi

API_CURRENCY_TEST=$(curl -s -X POST "http://localhost/api/convert-currency" \
    -H "Content-Type: application/x-www-form-urlencoded" \
    -d "amount=100&from=USD&to=EUR" \
    -w "%{http_code}" -o /dev/null 2>/dev/null || echo "000")

if [[ "$API_CURRENCY_TEST" =~ ^[23] ]]; then
    log "✓ Currency API working (HTTP $API_CURRENCY_TEST)"
else
    warning "Currency API status: HTTP $API_CURRENCY_TEST"
fi
